...
Paste this into a coding agent session at the root of a project that was **previously initialized** with the scaffold. A new version of the scaffold source has been placed in the project directory. The source may be a ZIP archive, an extracted scaffold directory, or an already-installed scaffold layout copied into a staging directory. This meta-prompt will update the project's scaffolding while preserving your customizations.
...
- If the source does not contain prompt files, delete the original ZIP (if one was used), any temporary extraction directory, and the confirmed staging source directory if it is inside the project root and is not the project root itself.
...
- Delete the original ZIP (if one was used), any temporary extraction directory, and the confirmed staging source directory if it is inside the project root and is not the project root itself.

---

STEP 6  -  REVIEW PROTECTED FILES

For each protected file that exists in both the source map and the project:
1. Compare the existing version with the new version from the source map.
