...
- Test `clean_stale_espanso_files()` deletes `espansr.yml` and `espansr-launcher.yml` from non-canonical match dirs
...
**Files:** `espansr/integrations/espanso.py`
...
**Files:** `espansr/integrations/espanso.py`, `espansr/ui/main_window.py`
...
- Add Bash function to detect and remove stale `espansr.yml` and `espansr-launcher.yml` from non-canonical Espanso locations
