...
class TemplateManager:
    ...
    def iter_with_triggers(self) -> Generator[Template, None, None]:
        """Yield templates that have a non-empty trigger defined.

        Returns:
            Generator of Template objects with trigger fields set.
        """
    ...
    def list_folders(self) -> List[str]:
        ...
        for path in self.templates_dir.iterdir():
            if path.is_dir() and path.name not in (self.VERSIONS_DIR, "_meta"):
    ...
    def delete_folder(self, name: str) -> tuple[bool, str]:
        ...
        if templates_in_folder:
            return (
                False,
                f"Cannot delete folder '{name}' because it contains "
                f"{len(templates_in_folder)} template(s). Move or delete them first.",
            )
